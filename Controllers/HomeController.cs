...
namespace AzureSearch.Controllers
{
    ...
    private void SaveFacets(SearchData model, bool saveChecks = false)
    {
        ...
        for (int j = 0; j < model.facetCat.Length; j++)
            {
                TempData["facet" + j.ToString()] = model.facetCat[j];
                if (saveChecks)
                {
                    TempData["facetCateg" + j.ToString()] = model.facetCateg[j];
                }
            }
            TempData["facetcount"] = model.facetCat.Length;
        }
